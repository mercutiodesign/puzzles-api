...
unsafe extern "C" fn memswap(
    ...
    while size > 0 as libc::c_int as size_t {
        let mut thissize: size_t =
            if size < ::core::mem::size_of::<[libc::c_char; 4096]>() as libc::c_ulong {
                size
            } else {
...
unsafe extern "C" fn downheap(
    ...
    while (2 as libc::c_int as size_t * i).wrapping_add(1 as libc::c_int as size_t) < nmemb {
        let mut j: size_t = i;
        if cmp.expect("non-null function pointer")(
            (array as *mut libc::c_char).offset((size * j) as isize) as *const libc::c_void,
            (array as *mut libc::c_char).offset(
                (size * (2 as libc::c_int as size_t * i).wrapping_add(1 as libc::c_int as size_t))
                    as isize,
            ) as *const libc::c_void,
            ctx,
        ) < 0 as libc::c_int
        {
            j = (2 as libc::c_int as size_t * i).wrapping_add(1 as libc::c_int as size_t);
        }
        if (2 as libc::c_int as size_t * i).wrapping_add(2 as libc::c_int as size_t) < nmemb
            && cmp.expect("non-null function pointer")(
                (array as *mut libc::c_char).offset((size * j) as isize) as *const libc::c_void,
                (array as *mut libc::c_char).offset(
                    (size
                        * (2 as libc::c_int as size_t * i).wrapping_add(2 as libc::c_int as size_t))
                        as isize,
                ) as *const libc::c_void,
                ctx,
            ) < 0 as libc::c_int
        {
            j = (2 as libc::c_int as size_t * i).wrapping_add(2 as libc::c_int as size_t);
        }
        if j == i {
            return;
        }
        memswap(
            (array as *mut libc::c_char).offset((size * j) as isize) as *mut libc::c_void,
            (array as *mut libc::c_char).offset((size * i) as isize) as *mut libc::c_void,
            size,
        );
        i = j;
    }
...
pub unsafe extern "C" fn arraysort_fn(
    ...
    while i > 0 as libc::c_int as size_t {
        memswap(
            (array as *mut libc::c_char).offset((size * 0 as libc::c_int as size_t) as isize)
                as *mut libc::c_void,
            (array as *mut libc::c_char).offset((size * i) as isize) as *mut libc::c_void,
            size,
        );
