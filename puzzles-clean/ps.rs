...
unsafe extern "C" fn ps_printf(mut ps: *mut psdata, mut fmt: *const libc::c_char, mut args: ...) {
    ...
    while i < npoints {
        ps_printf(
            ...
            *coords.offset((i * 2 as libc::c_int) as isize),
            (*ps).ytop - *coords.offset((i * 2 as libc::c_int + 1 as libc::c_int) as isize),
        );
        i += 1;
        i;
    }
...
unsafe extern "C" fn ps_line_width(mut handle: *mut libc::c_void, mut width: libc::c_float) {
    let mut ps: *mut psdata = handle as *mut psdata;
    ps_printf(
        ps,
        b"%g setlinewidth\n\0" as *const u8 as *const libc::c_char,
        ...
        while *p != 0 {
            ...
            if c < 0x80 as libc::c_int {
                ...
                *fresh1 = c as libc::c_char;
                ...
                let fresh3 = q;
                q = q.offset(1);
                *fresh3 = (c << 6 as libc::c_int | *fresh2 as libc::c_int & 0x3f as libc::c_int)
                    as libc::c_char;
            }
        }
        if *p == 0 {
...
unsafe extern "C" fn ps_begin_doc(mut handle: *mut libc::c_void, mut pages: libc::c_int) {
    let mut ps: *mut psdata = handle as *mut psdata;
    fputs(
        b"%!PS-Adobe-3.0\n\0" as *const u8 as *const libc::c_char,
        (*ps).fp,
...
unsafe extern "C" fn ps_begin_page(mut handle: *mut libc::c_void, mut number: libc::c_int) {
    let mut ps: *mut psdata = handle as *mut psdata;
    fprintf(
        (*ps).fp,
        b"%%%%Page: %d %d\ngsave save\n%g dup scale\n\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn ps_begin_puzzle(
    ...
    (*ps).gamewidth = pw;
    (*ps).gameheight = ph;
    (*ps).hatchthick = (0.2f64 * pw as libc::c_double / wmm as libc::c_double) as libc::c_float;
    (*ps).hatchspace = (1.0f64 * pw as libc::c_double / wmm as libc::c_double) as libc::c_float;
}
...
unsafe extern "C" fn ps_end_page(mut handle: *mut libc::c_void, mut number: libc::c_int) {
    let mut ps: *mut psdata = handle as *mut psdata;
    fputs(
...
static mut ps_drawing: drawing_api = unsafe {
    {
        let mut init = drawing_api {
            ...
            blitter_load: None,
            begin_doc: Some(
                ps_begin_doc as unsafe extern "C" fn(*mut libc::c_void, libc::c_int) -> (),
            ),
            begin_page: Some(
                ps_begin_page as unsafe extern "C" fn(*mut libc::c_void, libc::c_int) -> (),
            ),
            begin_puzzle: Some(
                ps_begin_puzzle
                    as unsafe extern "C" fn(
            ...
            end_puzzle: Some(ps_end_puzzle as unsafe extern "C" fn(*mut libc::c_void) -> ()),
            end_page: Some(
            ...
            line_width: Some(
                ps_line_width as unsafe extern "C" fn(*mut libc::c_void, libc::c_float) -> (),
            ),
            line_dotted: Some(
...
#[no_mangle]
pub unsafe extern "C" fn ps_init(mut outfile: *mut FILE, mut colour: bool) -> *mut psdata {
    let mut ps: *mut psdata =
        smalloc(::core::mem::size_of::<psdata>() as libc::c_ulong) as *mut psdata;
    (*ps).fp = outfile;
    (*ps).colour = colour;
